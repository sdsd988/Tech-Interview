...
정의 : DHCP(Dynamic Host Configuration Protocol)**는 네트워크에서 IP 주소를 자동으로 할당하고 관리하는 프로토콜입니다. 이를 통해 네트워크에 연결된 장치들이 수동으로 IP 주소를 설정하지 않고도 자동으로 네트워크 설정을 받을 수 있습니다
...
- DHCP는 클라이언트(PC, 스마트폰 등)가 네트워크에 접속할 때 IP 주소와 네트워크 설정 정보를 자동으로 할당하는 과정을 거칩니다.
